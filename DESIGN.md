...
| Skills | `skills/**/SKILL.md` + `scripts/*.js` | 知识型 skill、脚本型 tool、命令生成输入 |
...
- `~/.codex/AGENTS.md`
- `~/.codex/skills/`
- `~/.agents/skills/<pack>/`（外部 pack runtime，如 gstack）

这里有两个事实必须同时记住：

1. Codex 已不再依赖 legacy `prompts/` 目录。
2. 安装阶段仍会写入 `AGENTS.md`，用于组合 persona 与 style 的运行时 guidance。
...
`skills/**/SKILL.md` frontmatter 是 skill metadata 的唯一事实源。命令生成、skill 校验、脚本执行都应基于同一份 frontmatter，而不是额外维护平行清单。
