---
name: red-team
description: 红队攻击参考。攻击模拟方法论、防御验证技术、检测规则开发依据。用于红队演练、CTF竞赛、防御体系验证。当用户提到红队、PoC、安全模拟、横向移动、权限验证、防御测试时使用。
---
...
> 以下 C2 工具用于红队演练中的远程管理。蓝队应熟悉这些工具的流量特征以建立检测规则。
...
### Metasploit
```bash
# [Security Testing] 生成 Payload（替换为测试 IP）
...
### 简易 HTTP C2（教学用）
```python
# [Security Testing] 教学用最小 C2 示例，演示 beacon 通信原理
...
### Pass-the-Hash (PTH)
```bash
# [Security Testing] Impacket（替换为目标）
...
### 基础免杀
```python
# [Security Testing] 常见混淆手法——蓝队需针对性建立检测规则
...
### Shellcode 加载（教学用）
```python
# [Security Testing] 演示 Windows shellcode 加载原理——蓝队检测重点
import ctypes
...
### 隐蔽通信
```python
# [Security Testing] 演示隐蔽信道原理——蓝队需监控这些异常流量模式
...
### Windows
```powershell
# [Security Testing] 常见持久化手法——蓝队需逐一建立检测规则
